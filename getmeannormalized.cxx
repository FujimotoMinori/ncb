int getmeannormalized() {
    ...
    for (int i=0; i < n; i++){//bin loop
        ...
        if (pj->GetMean() != 0.0) {
            ...
            if(bingo){
							  //cout << "flag = " << flag << endl;
							  //cout << "mean = " << pj->GetMean() << endl;
                y.push_back(pj->GetMean()/flag);
                yc.push_back(pjc->GetMean()/flag);
                ye.push_back(pj->GetMeanError()/flag);
                yec.push_back(pjc->GetMeanError()/flag);
            }else{
                y.push_back(0.);
                yc.push_back(0.);
                ye.push_back(0.);
    ...
    tg->GetYaxis()->SetRangeUser(0.,1.3);//420,720
    tgc->SetMarkerStyle(20);
    tgc->SetMarkerColor(kRed);
    tgc->SetMarkerSize(0.5);
    tgc->GetYaxis()->SetRangeUser(0.,1.3);//420,720
